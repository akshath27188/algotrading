...
positions = []
def execute_trade(pdf,signal, current_price):
    ...
    if signal == 'Buy' and available_capital >= trade_cost and len(positions) == 0:
        ...
        for pos in positions:
            if pos['type'] == 'Buy':
                price_diff = (current_price - pos['price'])
                meets_threshold = price_diff / pos['price'] >= profit_threshold
                if meets_threshold:
                    profit += (current_price - pos['price']) * pos['quantity']
        ...
        for pos in positions:
            if pos['type'] == 'Sell':
                price_diff = (pos['price'] - current_price)
                meets_threshold = price_diff / pos['price'] >= profit_threshold
                if meets_threshold:
                    profit += (pos['price'] - current_price) * pos['quantity']
                    available_capital += trade_cost + profit
...
def process_data(pdf,backtest):
    ...
    df = execute_trade(pdf,signal, current_price)
    return df
...
max_window=40
def beginCollectiveTABasedStrategy(client, symbol_map,symbol):
    column_to_use = 'Close' if backtest else 'Ltp' 
    df = pd.DataFrame(columns=["Date", column_to_use, "Signal", "PnL"])
    #pd.set_option('display.max_columns', None)
    while True:
        ...
        if len(df) >= max_window:
            ...
            print('signal-',signal)
            logging.info(wdf)
            processed_df = process_data(wdf,False) #, ltp_price, signal)
            print('Length of df after process_data( shows value only if there is a signal generated)- ',len(processed_df))
...
try:
    ...
    #live run
    #beginCollectiveTABasedStrategy(client, symbols_map,symbol)
    ...
    #backtest strategy
    backtest_results = backtest_intraday_strategy_with_profit_threshold(symbol+'.NS', '2024-12-02', interval='1m', quantity=quantity_per_trade, profit_threshold=profit_threshold)
